...
class AiPlayer(object):
    ...
    def __init__(self, name, identifier):
        ...
        #### self.MaxSimulation = Int32.Parse(ConfigurationManager.AppSettings["AiMaxIteration"]) 파이썬으로 어떻게 해야하는지 모르겠어요.
    ...
    def NextMove(self, currentBoard):
        ...
        for i in range(7):
            ...
            return sorted(self.results, reverse=True, key=lambda x:x[1])  # 결과 리스트를 리스트 요소의 Value 항목에 대한 내림차순으로 정렬하여 가장 첫번째 요소의 Column 항목을 반환한다. 즉, 가장 높은 휴리스틱 값을 가지는 경우의 수는 어떤 열을 선택한 것인지 반환
    ...
    def Simulate(self, currentBoard):
        ...
        currentState = BoardState()  #### 클라스 참조하는법.....?
